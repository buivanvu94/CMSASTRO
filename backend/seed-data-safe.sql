-- ============================================
-- SEED SAMPLE DATA - SAFE VERSION
-- Tự động tạo user nếu chưa có, sau đó mới insert data
-- ============================================

-- Sử dụng user id = 3 (user hiện tại của bạn) làm author cho posts
-- Không cần tạo user mới vì đã có sẵn

-- ============================================
-- 1. CATEGORIES (8 items)
-- ============================================

-- Categories cho Posts (4 items)
INSERT INTO categories (name, slug, description, type, sort_order, status, seo_title, seo_description, created_at, updated_at) VALUES
('Tin tức', 'tin-tuc', 'Các tin tức mới nhất về nhà hàng và ẩm thực', 'post', 1, 'active', 'Tin tức - Nhà hàng', 'Cập nhật tin tức mới nhất về nhà hàng, sự kiện và khuyến mãi', NOW(), NOW()),
('Công thức nấu ăn', 'cong-thuc-nau-an', 'Chia sẻ các công thức nấu ăn đặc sắc', 'post', 2, 'active', 'Công thức nấu ăn', 'Khám phá các công thức nấu ăn ngon và dễ làm', NOW(), NOW()),
('Sự kiện', 'su-kien', 'Thông tin về các sự kiện và chương trình đặc biệt', 'post', 3, 'active', 'Sự kiện nhà hàng', 'Tham gia các sự kiện và chương trình đặc biệt tại nhà hàng', NOW(), NOW()),
('Khuyến mãi', 'khuyen-mai', 'Các chương trình khuyến mãi hấp dẫn', 'post', 4, 'active', 'Khuyến mãi', 'Cập nhật các chương trình khuyến mãi và ưu đãi đặc biệt', NOW(), NOW());

-- Categories cho Products (4 items)
INSERT INTO categories (name, slug, description, type, sort_order, status, seo_title, seo_description, created_at, updated_at) VALUES
('Món khai vị', 'mon-khai-vi', 'Các món khai vị ngon miệng để bắt đầu bữa ăn', 'product', 1, 'active', 'Món khai vị', 'Thưởng thức các món khai vị đặc sắc tại nhà hàng', NOW(), NOW()),
('Món chính', 'mon-chinh', 'Các món ăn chính phong phú và đa dạng', 'product', 2, 'active', 'Món chính', 'Khám phá thực đơn món chính đa dạng và hấp dẫn', NOW(), NOW()),
('Tráng miệng', 'trang-mieng', 'Các món tráng miệng ngọt ngào', 'product', 3, 'active', 'Món tráng miệng', 'Kết thúc bữa ăn với các món tráng miệng tuyệt vời', NOW(), NOW()),
('Đồ uống', 'do-uong', 'Các loại đồ uống giải khát và cocktail', 'product', 4, 'active', 'Đồ uống', 'Thưởng thức các loại đồ uống đa dạng và sảng khoái', NOW(), NOW());
...
(
  (SELECT id FROM categories WHERE slug = 'cong-thuc-nau-an' LIMIT 1),
  3,
...
(
  (SELECT id FROM categories WHERE slug = 'su-kien' LIMIT 1),
  3,
...
(
  (SELECT id FROM categories WHERE slug = 'khuyen-mai' LIMIT 1),
  3,
...
(
  (SELECT id FROM categories WHERE slug = 'mon-khai-vi' LIMIT 1),
  'Gỏi cuốn tôm thịt',
  'goi-cuon-tom-thit',
...
(
  (SELECT id FROM categories WHERE slug = 'mon-chinh' LIMIT 1),
  'Bò bít tết Úc',
  'bo-bit-tet-uc',
...
(
  (SELECT id FROM categories WHERE slug = 'trang-mieng' LIMIT 1),
